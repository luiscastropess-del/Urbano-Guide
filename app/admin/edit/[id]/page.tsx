...
export default function EditPlacePage() {
  ...
  const profileInputRef = useRef<HTMLInputElement>(null); // Novo
  ...
  const handleProfileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      showToast('🖼️ Logo/Avatar atualizado com sucesso');
    }
  };
  ...
  if (authLoading || fetching) {
    return (
      <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-900 overflow-hidden items-center justify-center p-8">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-orange-500"></div>
        <p className="mt-4 text-slate-500 font-medium">Autenticando e carregando dados...</p>
      </div>
    );
  }
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 border border-white/20 rounded-2xl p-4 mb-5 shadow-sm">
              ...
              <div className="flex gap-4">
                <div className="h-24 w-24 shrink-0 rounded-xl bg-gradient-to-br from-amber-200 to-orange-300 flex items-center justify-center text-3xl shadow-md relative overflow-hidden" onClick={() => coverInputRef.current?.click()}>
                  {formData.coverImage ? (
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 border border-white/20 rounded-2xl p-4 mb-5 shadow-sm">
              <h3 className="font-bold text-sm mb-3 flex items-center gap-2">
                <ImageIcon size={16} className="text-blue-500" /> Logo / Imagem de Perfil
              </h3>
              <p className="text-xs text-slate-500 mb-3">Imagem que aparece no lugar do Emoji.</p>
              
              <div className="flex gap-4">
                <div className="h-24 w-24 shrink-0 rounded-full bg-gradient-to-br from-blue-200 to-cyan-300 flex items-center justify-center text-3xl shadow-md relative overflow-hidden border-4 border-white dark:border-slate-800" onClick={() => profileInputRef.current?.click()}>
                  {formData.profileImage ? (
                    <img src={formData.profileImage} className="w-full h-full object-cover" />
                  ) : (
                    formData.emoji || '🏢'
                  )}
                  <button type="button" className="absolute -top-1 -right-1 h-6 w-6 rounded-full bg-blue-500 text-white flex items-center justify-center text-xs shadow-md">
                    <Edit2 size={10} />
                  </button>
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <p className="text-xs text-slate-500 mb-1">{formData.profileImage ? 'Personalizada' : 'Usando emoji'}</p>
                  <button type="button" className="text-xs text-blue-500 flex items-center gap-1 w-fit" onClick={() => profileInputRef.current?.click()}>
                    <RefreshCw size={12} /> Trocar imagem
                  </button>
                </div>
              </div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="bg-white/70 dark:bg-slate-800/70 border border-white/20 rounded-2xl p-4 mb-5 shadow-sm">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="h-14 w-14 shrink-0 rounded-2xl bg-gradient-to-br from-amber-200 to-orange-300 flex items-center justify-center text-2xl shadow-sm border border-white/50 overflow-hidden">
                  {formData.profileImage ? <img src={formData.profileImage} className="w-full h-full object-cover" /> : formData.emoji}
                </div>
