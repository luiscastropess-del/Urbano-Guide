...
export default function UltimateGuideProfileClient({ guide }: { guide: any }) {
  ...
  const profileImage = guide.user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&size=200&background=a855f7&color=fff`;
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .ultimate-gradient { background: linear-gradient(135deg, #a855f7 0%, #6366f1 30%, #8b5cf6 60%, #d946ef 100%); }
        .holambra-gradient { background: linear-gradient(135deg, #f97316 0%, #22c55e 100%); }
        .tab-button { padding: 10px 20px; font-weight: 500; font-size: 14px; border-radius: 40px; transition: all 0.2s; cursor: pointer; }
        .tab-button.active { background: #a855f7; color: white; }
        .feed-scroll::-webkit-scrollbar { width: 4px; }
        .feed-scroll::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
        .ultimate-badge { background: linear-gradient(135deg, #a855f7, #6366f1, #d946ef); color: white; font-size: 11px; font-weight: 600; padding: 4px 12px; border-radius: 40px; display: inline-flex; align-items: center; box-shadow: 0 4px 15px rgba(168,85,247,0.4); }
      ...
      <div className="max-w-md mx-auto relative h-[calc(100vh-60px)] md:h-screen flex flex-col shadow-2xl overflow-hidden bg-slate-50 dark:bg-slate-950">
        <div className="relative shrink-0">
          <div className="h-48 bg-gradient-to-br from-violet-200 via-fuchsia-200 to-amber-200 dark:from-violet-900/30 dark:via-fuchsia-900/20 dark:to-amber-900/30 relative overflow-hidden">
            ...
            <div className="absolute bottom-4 right-4 ultimate-badge z-10">
              <i className="fas fa-crown mr-1 text-yellow-300"></i> ULTIMATE
            </div>
          </div>
          <div className="absolute -bottom-8 left-5 z-20">
            <div className="h-20 w-20 rounded-2xl ultimate-gradient p-0.5 shadow-xl relative">
            ...
            <div className="absolute -top-1 -right-1 bg-purple-600 text-white rounded-full h-8 w-8 flex items-center justify-center shadow-md text-xs z-30">
        ...
        <div className="flex-1 overflow-y-auto feed-scroll px-5 pt-10 pb-20 relative z-10">
          <div className="flex items-start justify-between mb-1">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="ultimate-badge"><i className="fas fa-crown mr-1 text-yellow-300"></i> ULTIMATE</span>
              </div>
          ...
          <div className="flex items-center gap-3 mt-3 mb-4 flex-wrap">
            ...
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
              <i className="fas fa-suitcase-rolling text-purple-500 mr-1"></i> {guide.totalKm || 860}+ viagens
            </span>
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full">
          ...
          <div className="glass-card rounded-2xl p-4 mb-5">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2 text-slate-900 dark:text-white"><i className="fas fa-shield-alt text-purple-500"></i> Segurança e Documentação</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-xs text-slate-500 dark:text-slate-400">📄 CADASTUR</span><p className="font-medium text-slate-900 dark:text-white">12.345-6</p></div>
              ...
              <div><span className="text-xs text-slate-500 dark:text-slate-400">🛡️ Seguro Turismo</span><p className="verified-badge w-fit mt-1">Vigente</p></div>
            </div>
          </div>

          <div className="glass-card rounded-2xl p-4 mb-5">
            <h3 className="font-bold text-sm mb-3 flex items-center gap-2 text-slate-900 dark:text-white"><i className="fas fa-car text-purple-500"></i> Veículo e Equipamentos</h3>
            <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="flex gap-3 mb-6">
            <button className="flex-1 py-3.5 holambra-gradient hover:opacity-90 text-white rounded-xl font-semibold shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2 transition-all active:scale-95" onClick={() => showToast('📅 Agendamento online')}>
              <i className="fas fa-calendar-plus"></i> Agendar Tour
            </button>
            <button className="w-14 h-14 shrink-0 rounded-xl bg-purple-600 hover:bg-purple-700 text-white flex items-center justify-center shadow-lg shadow-purple-500/20 transition-all active:scale-95" onClick={() => showToast('💬 Chat com o guia')}>
          ...
          <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar pb-1 text-slate-600 dark:text-slate-300">
            ...
            <button className={`tab-button whitespace-nowrap ${activeTab === 'faq' ? 'active text-white' : 'bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700'}`} onClick={() => setActiveTab('faq')}>❓ FAQ</button>
          </div>

          {activeTab === 'roteiros' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="space-y-3 mb-6">
                <div className="glass-card rounded-xl p-4 bg-gradient-to-r from-purple-50 to-fuchsia-50 dark:from-purple-950/20 dark:to-fuchsia-950/20 border border-purple-200 dark:border-purple-800">
                ...
                <div className="glass-card rounded-xl p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-slate-900 dark:text-white">🌅 Tour do Por do Sol</h4>
                      <p className="text-xs text-slate-500">2h • 5 km • Fácil</p>
                    </div>
                    <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full">Ativo</span>
                  </div>
                </div>
                <div className="glass-card rounded-xl p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">4h • Inclui fotógrafo • Grupos até 6</p>
                    </div>
                    <span className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded-full">Ativo</span>
                  </div>
                </div>
              </div>
              <p className="text-xs text-slate-400 text-center font-medium">Roteiros ilimitados · ULTIMATE</p>
            </div>
          )}
          ...
          {activeTab === 'pacotes' && (
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="space-y-3 mb-6">
                <div className="glass-card rounded-xl p-4 bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800">
                  <div className="flex justify-between items-start">
                    <div>
                  ...
                  <p className="font-black text-purple-600 dark:text-purple-500 mt-2 text-lg">R$ 250 <span className="text-xs text-slate-500 font-medium">/ pessoa</span></p>
                </div>
                ...
                <div className="glass-card rounded-xl p-4 bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800">
                  ...
                  <p className="font-black text-purple-600 dark:text-purple-500 mt-2 text-lg">R$ 320 <span className="text-xs text-slate-500 font-medium">/ pessoa</span></p>
                </div>
              </div>
              <p className="text-xs text-slate-400 text-center font-medium">Pacotes em destaque ilimitados · ULTIMATE</p>
            </div>
          )}
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="space-y-3 mb-6">
                <div className="glass-card rounded-xl p-4 flex items-center gap-3">
                  ...
                  <button className="text-purple-500 hover:text-purple-600 transition ml-2" onClick={() => showToast('▶️ Reproduzindo vídeo...')}>
                ...
                <div className="glass-card rounded-xl p-4 flex items-center gap-3">
                  <div className="h-16 w-16 rounded-xl bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-2xl shrink-0">🎬</div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-sm text-slate-900 dark:text-white truncate">Tour 360° · Holambra Virtual</p>
                    <p className="text-xs text-slate-500 mt-0.5">4:12 min</p>
                  </div>
                  <button className="text-purple-500 hover:text-purple-600 transition ml-2" onClick={() => showToast('▶️ Reproduzindo tour 360°...')}>
                    <i className="fas fa-play-circle text-3xl"></i>
                  </button>
                </div>
              </div>
                ...
                {[
                  ...
                  <div key={index} className="glass-card rounded-xl p-4">
                    <div className="flex items-center justify-between cursor-pointer" onClick={() => toggleFaq(index)}>
                      ...
                      <button className="text-purple-500"><i className={`fas fa-chevron-${openFaqIndex === index ? 'up' : 'down'} text-xs`}></i></button>
                    </div>
                    {openFaqIndex === index && (
                      <p className="text-xs text-slate-600 dark:text-slate-400 mt-3 animate-in fade-in slide-in-from-top-1">{faq.a}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="glass-card rounded-2xl p-4 mt-6 mb-4 bg-gradient-to-r from-purple-50 to-fuchsia-50 dark:from-purple-950/20 dark:to-fuchsia-950/20 border border-purple-200 dark:border-purple-800">
            <h3 className="font-bold text-sm flex items-center gap-2 text-slate-900 dark:text-white"><i className="fas fa-crown text-purple-500"></i> Você está visualizando o plano Ultimate</h3>
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Este guia possui visibilidade máxima, roteiros ilimitados e recursos exclusivos.</p>
            <button className="w-full mt-4 py-2.5 bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-700 hover:to-fuchsia-700 text-white rounded-full text-sm font-bold shadow-md transition-all active:scale-95" onClick={() => showToast('👑 Plano Ultimate ativo')}>
              Máximo desempenho
            </button>
          </div>
